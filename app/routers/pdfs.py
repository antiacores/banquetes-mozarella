...
def generar_pdf_trabajadores(nombre, fecha, articulos, articulos_cajon=None):
    ...
    row_h = [0.75*cm] + [1.1*cm] * len(filas)
    ...
    if articulos_cajon:
        ...
        row_h_c = [0.6*cm] + [0.9*cm] * len(filas_c)
        estilo_c = TableStyle([
            ("FONTNAME",  (0, 0), (-1,  0), "Helvetica"),
            ("FONTSIZE",  (0, 0), (-1,  0), 6),
            ("TEXTCOLOR", (0, 0), (-1,  0), GRIS_T),
...
def generar_pdf_cotizacion(nombre, fecha, articulos, extras_filas, articulos_cajon=None):
    ...
    total_arts   = sum(a["cantidad"] * float(a["precio_unitario"]) for a in todos)
...
def pdf_evento_trabajadores(id_evento: int, db: Session = Depends(get_db)):
    ...
    articulos_cajon = [
        {"nombre": c.nombre, "cantidad_asignada": c.cantidad}
        for c in cajon
    ] if cajon else None
    ...
    articulos_cajon = [
        {
            "nombre": c.nombre,
            "cantidad": c.cantidad,
            "precio_unitario": float(c.precio) if c.precio else 0.0,
        }
        for c in cajon
    ] if cajon else None
