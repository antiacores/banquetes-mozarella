...
def _extras_a_filas(payload: CotizacionPayload):
    filas = []
    if payload.flete > 0:
        filas.append({"nombre": "Flete", "monto": payload.flete})
    if payload.pista > 0:
        filas.append({"nombre": "Pista", "monto": payload.pista})
    if payload.carpa > 0:
        filas.append({"nombre": "Carpa", "monto": payload.carpa})
    if payload.cocteles > 0:
        filas.append({"nombre": "Cócteles", "monto": payload.cocteles})
    for e in payload.extras:
        if e.monto > 0:
            filas.append({"nombre": e.nombre, "monto": e.monto})
    return filas


# ── Logo y colores (sin cambios) ──────────────────────────────────────────
...
def generar_pdf_cotizacion(nombre, fecha, articulos, extras_filas):
    """
    articulos:    lista de dicts {nombre, cantidad, precio_unitario}
    extras_filas: lista de dicts {nombre, monto}
                  incluye flete, pista, carpa, cocteles y extras libres
    """
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=letter)
    W, H = letter
    ...
    total_extras = sum(e["monto"] for e in extras_filas)
    total_final  = total_arts + total_extras

    filas_extras = [
        ["", "", e["nombre"].upper(), f"$  {e['monto']:,.2f}"]
        for e in extras_filas
    ]
    ...
    row_h = [0.75*cm] + [0.6*cm] * len(filas) + [0.65*cm] * n_extras + [0.8*cm]
    n = len(datos)
    ...
    tabla.setStyle(TableStyle([
        ...
        ("FONTSIZE",  (0, 1), (-1, n - n_extras - 2), 7.5),
...
def pdf_evento_trabajadores(id_evento: int, db: Session = Depends(get_db)):
    ...
    buf = generar_pdf_trabajadores(
        evento.nombre_cliente or "Sin nombre", fecha_fmt, articulos
    )
    return StreamingResponse(buf, media_type="application/pdf",
        headers={"Content-Disposition": f"inline; filename=evento_{id_evento}_trabajadores.pdf"})


@router.post("/evento/{id_evento}/cotizacion")
def pdf_evento_cotizacion(
    id_evento: int,
    ...
    buf = generar_pdf_cotizacion(
        evento.nombre_cliente or "Sin nombre", fecha_fmt, articulos, extras_filas
    )
    return StreamingResponse(buf, media_type="application/pdf",
...
@router.post("/renta/{id_renta}/cotizacion")
def pdf_renta_cotizacion(
    id_renta: int,
    payload: CotizacionPayload,
    db: Session = Depends(get_db),
):
